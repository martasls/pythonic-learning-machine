...
from benchmark.configuration import get_random_config_slm_fls_grouped, get_random_config_slm_ols_grouped, \
    ...
    get_config_mlp_adam, get_config_mlp_sgd, get_random_config_xcs
...
XCS = {
    ...
    'xcs': {
        'name_long': 'Accuracy-based Learning Classifier System',
        'name_short': 'XCS', 
        'algorithms': [EvaluatorXCS], 
        'configuration_method': get_random_config_xcs, 
        'max_combinations': _XCS_MAX_COMBINATIONS
    }
}
...
class Benchmarker():
    ...
    def _evaluate_algorithm_xcs(self, algorithm, configurations, training_set, validation_set, testing_set, metric):
        """Creates XCS evaluator, based on the algorithms and configurations """
        evaluator_xcs = algorithm(configurations, training_set, validation_set, testing_set, metric)
        return evaluator_xcs.run_nested_cv_xcs()
    ...
    def _set_initial_validation_value(self):
        if self.classification: 
        ...
        return best_overall_validation_value

    def run_nested_cv_xcs(self):
        """ runs benchmark study for XCS on a nested CV environment"""
        
        print('Entering run_nested_cv_xcs for dataset:', self.data_set_name)
        ...
        for training_outer_index, testing_index in outer_folds.split(get_input_variables(self.samples).values, get_target_variable(self.samples).values):
            ...
            best_overall_validation_value = self._set_initial_validation_value()

            for key in self.models.keys(): 
                print('\t\tAlgorithm with key:', key)
                if not self.results[key][outer_cv]:
                    best_validation_value = self._set_initial_validation_value()
                    ...
                    self.results[key][outer_cv] = self._evaluate_algorithm_xcs(algorithm=best_algorithm, configurations=best_configuration,
            ...
            self.best_result[outer_cv] = self._evaluate_algorithm_xcs(algorithm=best_overall_algorithm, configurations=best_overall_configuration,
                                                        training_set=training_outer, validation_set=None, testing_set=testing, metric=self.metric)
