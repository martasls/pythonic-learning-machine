...
def process_all_data_sets():
    ...
    [_to_pickle(data_samples_list, data_set_name) for data_samples_list, data_set_name in
     zip(
         [_resample_data_set_iter(data_set, 0.5, 0.3, 0.2) for data_set in #changed from 0.5, 0.3, 0.2 to 0.7, 0, 0.3
...
def _resample_data_set_no_val(data_set, train_frac=None, test_frac=None):
    """"""
    assert len([frac for frac in [train_frac, test_frac] if not frac]) <= 1
    assert sum([frac for frac in [train_frac, test_frac] if frac]) <= 1
...
def _resample_data_set_iter_no_val(data_set, train_frac=None, test_frac=None, iterations=30):
    """"""
    return list(_resample_data_set_no_val(data_set, train_frac, test_frac) for i in range(iterations))


def _to_pickle_sample_no_val(data_samples, data_set_name, index):

    file_path = join('04_resampled', data_set_name)

    file_names = [sample_name + "_" + str(index)
                  for sample_name in ['training', 'testing']]

    [data_set_to_pickle(data_sample, file_path, file_name)
     for data_sample, file_name in zip(data_samples, file_names)]


def _to_pickle_no_val(data_samples_list, data_set_name):
    [_to_pickle_sample_no_val(data_samples, data_set_name, index) for data_samples, index
     in zip(data_samples_list, range(len(data_samples_list)))]


def process_all_data_sets_no_val():
    """"""
    data_set_names = [remove_extension(file) for file in list_files('03_standardized')]
    [_to_pickle_no_val(data_samples_list, data_set_name) for data_samples_list, data_set_name in
     zip(
         [_resample_data_set_iter_no_val(data_set, 0.7, 0.3) for data_set in 
          [read_standardized_data_set(data_set_name) for data_set_name in data_set_names]],
         data_set_names)]

process_all_data_sets_no_val()